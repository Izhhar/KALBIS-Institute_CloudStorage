...
?>
		...
		<div class="row">
			<div class="col-xs-12 col-sm-8 col-md-8 col-sm-offset-2 col-md-offset-2">
				<?php
			$result = $connect->query("SELECT `id_user` FROM `users` WHERE `id_user` = '".$id_user."'");
	if($result->num_rows > 0)
	{
		if($row = $result->fetch_assoc())
		{?>
						<div class="row">
							<form id="change_password" action="doChangePassword.php?id_user=<?php echo $row['id_user']; ?>" method="POST">
									...
									<table class="table table-user-information">
										<tbody>
										  <tr>
											...
											<td>:</td>
											<td>
												<input class="form-control" name="old_password" type="password" autofocus required>
											</td>
										  </tr>
										  <tr>
											<td>New password</td>
